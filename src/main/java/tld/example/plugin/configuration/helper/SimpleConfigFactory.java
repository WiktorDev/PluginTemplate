package tld.example.plugin.configuration.helper;
...
public class SimpleConfigFactory {
    ...
    public SimpleConfigFactory(@NonNull File dataDirectory) {
    ...
    public <T extends OkaeriConfig> T produce(@NonNull Class<T> type, @NonNull File file) {
        return ConfigManager.create(type, it -> it.withConfigurer(new YamlBukkitConfigurer()).withBindFile(file).withSerdesPack(new SerdesBukkit()).saveDefaults().load(true));
    }
}